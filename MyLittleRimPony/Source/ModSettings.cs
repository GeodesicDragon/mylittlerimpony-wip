...
namespace MLRP_ModSettings
{
    ...
    public class MLRP_SettingsWindow : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            MLRP_Options.Label("v" + MLRP_Version);
            ...
            settings.TreeHuggerStuff = (int)MLRP_Options.Slider(settings.TreeHuggerStuff, 50, 500);
            ...
            if (ModsConfig.IsActive("Ludeon.RimWorld.Anomaly"))
            {
				...
				settings.ThingponeStuff = (int)MLRP_Options.Slider(settings.ThingponeStuff, 50, 500);
            }
        ...
        public static void MLRP_ApplySettings()
        {
            ...
            DefDatabase<RecipeDef>.GetNamed("MLRP_MakeElementsOfHarmony").ingredients[0].SetBaseCount(EOHCount);
            DefDatabase<RecipeDef>.GetNamed("MLRP_MakeCutieMarkCrusaders").ingredients[0].SetBaseCount(CMCCount);

            DefDatabase<RecipeDef>.GetNamed("MLRP_MakeElementsOfHarmony").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
            DefDatabase<RecipeDef>.GetNamed("MLRP_MakeElementsOfHarmonyAlternative").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
            DefDatabase<RecipeDef>.GetNamed("MLRP_MakeCutieMarkCrusadersAlternative").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
            ...
            {
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeUnicornTwilight").ingredients[0].SetBaseCount(BaseCount);
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeUnicornTwilight").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
            ...
            if (ModsConfig.IsActive("Dubwise.DubsBadHygiene"))
            {
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakePippPetals").ingredients[0].SetBaseCount(BaseCount);
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakePippPetals").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
            ...
            {
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeMeadowbrook").ingredients[0].SetBaseCount(BaseCount);
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeMeadowbrook").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
                DefDatabase<ThingDef>.GetNamed("PonyPlush_Meadowbrook").apparel.careIfWornByCorpse = settings.taintedOnDeath;
            }

            // UPDATE PLUSHIE STUFF COSTS

            foreach (string defName in CorePlushRecipes)
            {
                RecipeDef recipe = DefDatabase<RecipeDef>.GetNamed(defName);
                if (recipe != null && recipe.ingredients != null && recipe.ingredients.Count > 0)
                {
                    recipe.ingredients[0].SetBaseCount(BaseCount);
                }

                var req = recipe.skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting);
                if (req != null)
                {
                    req.minLevel = settings.craftingSkillRequirement;
                }
            }

            foreach (string defName in CorePlushies)
            {
                ThingDef thing = DefDatabase<ThingDef>.GetNamed(defName);
                thing.apparel.careIfWornByCorpse = settings.taintedOnDeath;
            }
            ...
            if (RecyleDef != null)
            {
                ...
                if (ext != null)
                {
                    ext.reclaimedAmount = settings.plushieRecycling; // whatever value you want from options
                }
            }
            ...
            {
                foreach (string defName in RoyaltyPlushRecipes)
                {
                    RecipeDef recipe = DefDatabase<RecipeDef>.GetNamed(defName);
                    if (recipe != null && recipe.ingredients != null && recipe.ingredients.Count > 0)
                    {
            ...
            if (ModsConfig.IsActive("Ludeon.RimWorld.Ideology"))
            {
                ...
                foreach (string defName in IdeologyPlushRecipes)
                {
                    RecipeDef recipe = DefDatabase<RecipeDef>.GetNamed(defName);
                    if (recipe != null && recipe.ingredients != null && recipe.ingredients.Count > 0)
                    {
            ...
            if (ModsConfig.IsActive("Ludeon.RimWorld.Biotech"))
            {
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeChildCutieMarkCrusaders").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeCutieMarkCrusadersChildAlternative").skillRequirements.FirstOrDefault(r => r.skill == SkillDefOf.Crafting).minLevel = settings.craftingSkillRequirement;
                DefDatabase<RecipeDef>.GetNamed("MLRP_MakeChildCutieMarkCrusaders").ingredients[0].SetBaseCount(CMCCountChild);
                ...
                else if (settings.SBMute == false)
                {
                    ...
                    foreach (string defName in BiotechPlushRecipes)
                    {
                        RecipeDef recipe = DefDatabase<RecipeDef>.GetNamed(defName);
                        if (recipe != null && recipe.ingredients != null && recipe.ingredients.Count > 0)
                        {
                ...
                foreach (string defName in ChildPlushRecipes)
                {
                    RecipeDef recipe = DefDatabase<RecipeDef>.GetNamed(defName);
                    if (recipe != null && recipe.ingredients != null && recipe.ingredients.Count > 0)
                    {
                        recipe.ingredients[0].SetBaseCount(ChildCount);
                    }
        ...
        private static readonly string[] CorePlushRecipes =
        {
            "MLRP_MakeAppleBloom",
            "MLRP_MakeApplejack",
            "MLRP_MakeBigMacintosh",
            "MLRP_MakeCheerilee",
            "MLRP_MakeDaringDo",
            "MLRP_MakeDiscord",
            "MLRP_MakeFleurDeLis",
            "MLRP_MakeFluttershy",
            "MLRP_MakeNurseHaywick",
            "MLRP_MakeIzzyMoonbow",
            "MLRP_MakeMaudPie",
            "MLRP_MakePinkiePie",
            "MLRP_MakePrincessCadence",
            "MLRP_MakePrincessCelestia",
            "MLRP_MakePrincessLuna",
            "MLRP_MakeRainbowDash",
            "MLRP_MakeRarity",
            "MLRP_MakeScootaloo",
            "MLRP_MakeShiningArmour",
            "MLRP_MakeSpoiledRich",
            "MLRP_MakeStarlightGlimmer",
            "MLRP_MakeSweetieBelle",
            "MLRP_MakeTwilightSparkle",
        };
        ...
        private static readonly string[] RoyaltyPlushRecipes =
        {
            "MLRP_MakeAloeAndLotus",
            "MLRP_MakeTrixie",
        };
        ...
        private static readonly string[] IdeologyPlushRecipes =
        {
            "MLRP_MakeFlimAndFlam",
            "MLRP_MakeKingSombra",
            "MLRP_MakeQueenChrysalis",
            "MLRP_MakeZecora",
        };
        ...
        private static readonly string[] BiotechPlushRecipes =
        {
            "MLRP_MakeButtonMash",
        };
        ...
        private static readonly string[] ChildPlushRecipes =
        {
            "MLRP_MakeChildAppleBloom",
            "MLRP_MakeChildCheerilee",
            "MLRP_MakeChildDiscord",
            "MLRP_MakeChildIzzyMoonbow",
            "MLRP_MakeChildPrincessCadence",
            "MLRP_MakeChildPrincessCelestia",
            "MLRP_MakeChildPrincessLuna",
            "MLRP_MakeChildRainbowDash",
            "MLRP_MakeChildScootaloo",
            "MLRP_MakeChildShiningArmour",
            "MLRP_MakeChildSpoiledRich",
            "MLRP_MakeChildStarlightGlimmer",
            "MLRP_MakeChildSweetieBelle",
            "MLRP_MakeCombatRainbowDash",
        };
